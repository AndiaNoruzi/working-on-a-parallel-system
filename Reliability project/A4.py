...
# Constants for model A4
alpha = [1e-5, 2.3e-5, 3e-5, 4e-5]
beta_prime = [1.28, 4.77, 2.13, 1.75]  # Changed beta values for A4
lambda_ = [alpha[i] / beta_prime[i] for i in range(4)]  # Calculate lambda_i for A4
...
def R_sys(n, t, lambda_):
    R = [R_i_exp(t, lambda_[i]) for i in range(4)]
...
data = {
    'ni': n_opt,
    'R(t)': [R_i_exp(t, lambda_[i]) for i in range(4)]
}
...
# Display results
print("Optimization Results for Model A4 with Integer Constraints:")
print(df)
